...
class SearchResults(Base):
    ...
    def __repr__(self):
        return (
            f"<SearchResults(path={self.path}, line={self.line}, "
...
async def search_files_from_gh(
    ...
    search_term: str = Field(description="The term to search for in the file"),
    save_to_db: bool = Field(description="Save the results to database.", default=False),
) -> str:
    """
    ...
    for path in paths_list:
        ...
        lines = r.text.splitlines()
        matches = [
            {"path": path, "line": i + 1, "search_term": search_term, "owner": owner.lower(), "repo": repo.lower()}
