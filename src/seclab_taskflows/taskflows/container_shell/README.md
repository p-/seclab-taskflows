...
Five container profiles are provided. Each has its own Dockerfile, toolbox
...
Source access and code exploration. Extends base with ripgrep, fd, tree,
universal-ctags, GNU global, and cscope. Does not include SAST scanners.
...
`CONTAINER_WORKSPACE` — host path to mount at `/workspace`. The source-access
and SAST toolboxes default this to `DATA_DIR/repo_under_test`, where
`local_gh_resources` extracts the repository currently under test, so only that
repo is mounted into the container.
...
server exits. The source-access and SAST toolboxes default this to `true` so
...
- The source-access and SAST toolboxes keep their containers alive by default
